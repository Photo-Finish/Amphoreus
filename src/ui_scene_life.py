...
# photo z0 → copy z5 → chat/widgets z40 → life sprites z55 → header/tabs
# z60–80 → look chrome z120 → land click-popup z400 (parent host).
_PAGE_PHOTO_Z = 0
_PAGE_COPY_Z = 5
_PAGE_LIFE_Z = 55
_PAGE_NOTICE_Z = 400
...
def pictorial_stage_html(
    ...
    *,
    ...
    is an ``<img>`` inside a low photo iframe; life sprites sit in a higher
    iframe so they paint above Streamlit dialogue (see ``_PAGE_LIFE_Z``).
    Click popups are hoisted to the parent (``_PAGE_NOTICE_Z``).
    """
...
def pictorial_stage_documents(
    ...
    *,
    ...
    pin_js = ""
    if page_layer:
        # Full page: JPEG at z=0 (behind copy). Life iframe paints at
        # _PAGE_LIFE_Z (above chat/wordings) but stays pointer-events:none
        # so wheel scroll is native. Sprite clicks and the notice card
        # are handled on the parent page.
        pin_js = (
    ...
    body = (
        ...
        "  function syncNoticeHost(){\n"
        "    try {\n"
        "      var pdoc = window.parent.document;\n"
        "      var host = pdoc.getElementById('amp-land-notice-host');\n"
        f"      var z = {int(_PAGE_NOTICE_Z)};\n"
        "      if (!card || card.hidden) {\n"
        "        if (host) { host.innerHTML = ''; host.style.display = 'none'; }\n"
        "        return;\n"
        "      }\n"
        "      if (!host) {\n"
        "        host = pdoc.createElement('div');\n"
        "        host.id = 'amp-land-notice-host';\n"
        "        pdoc.body.appendChild(host);\n"
        "      }\n"
        "      host.style.cssText = 'position:fixed;right:14px;top:14%;left:auto;bottom:auto;'\n"
        "        + 'z-index:'+z+';pointer-events:auto;max-width:min(300px,32vw);'\n"
        "        + 'width:max-content;';\n"
        "      host.style.display = 'block';\n"
        "      host.innerHTML = card.innerHTML;\n"
        "      var x = host.querySelector('.amp-notice-x');\n"
        "      if (x) x.addEventListener('click', function(ev){\n"
        "        ev.preventDefault(); ev.stopPropagation(); hideNotice();\n"
        "      });\n"
        "      var cloned = host.querySelectorAll('.amp-notice-act');\n"
        "      var srcActs = card.querySelectorAll('.amp-notice-act');\n"
        "      for (var i = 0; i < cloned.length; i++) {\n"
        "        (function(idx){\n"
        "          cloned[idx].addEventListener('click', function(ev){\n"
        "            ev.preventDefault(); ev.stopPropagation();\n"
        "            if (srcActs[idx]) srcActs[idx].click();\n"
        "            var line = host.querySelector('.amp-notice-line');\n"
        "            if (line && lineEl) line.textContent = lineEl.textContent;\n"
        "          });\n"
        "        })(i);\n"
        "      }\n"
        "    } catch (e) {}\n"
        "  }\n"
        "  function hideNotice(){ if (card) card.hidden = true; syncNoticeHost(); }\n"
        "  try {\n"
        "    window.addEventListener('message', function(ev){\n"
        "      if (ev && ev.data && ev.data.amp === 'hideNotice') hideNotice();\n"
        "    });\n"
        "  } catch (e) {}\n"
